...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        if [[ -n "${GITHUB_TOKEN}" ]]; then
            ...
            echo -e "${red}GitHub token is missing, please set the GITHUB_TOKEN environment variable.${plain}"
            exit 1
        fi

        last_version=$(curl -H "${auth_header}" -Ls "https://api.github.com/repos/MHSanaei/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
